...
class Solution {
public:
    int subsets(vector<int> &nums, int n, int ind, int xorr) {
        if (ind == n) {
            return xorr;
        }

        int include = subsets(nums, n, ind+1, xorr ^ nums[ind]);
        int exclude = subsets(nums, n, ind+1, xorr);
