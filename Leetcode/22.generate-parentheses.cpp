...
public:
    ...
    void parenthesis(int n, vector<string> &res, string com, int a, int b) {
        if (a == n && b == n) {
            res.push_back(com);
            return;
        }

        // what is we have to add (
        if (a != n) {
            com += "(";
            parenthesis(n, res, com, a+1, b);

            com.pop_back();
        }

        // what is we need to add )
        if (b != n && b < a) {
            com += ")";
            parenthesis(n, res, com, a, b+1);
